...
- canonical agents/skills plus runtime mirrors: `.claude/`, `.codex/`, `.cursor/`, `openclaw/`, installed/global inventories, package scripts, local scripts, commands, hooks, validators, prompts/rules, setup/status/doctor/sync/install routes
- `.mcp.json`, runtime MCP config, MCP server/tool inventory, and connector/plugin inventories
