...
Fan-out eligibility is only the complexity decision; live host-agent dispatch also requires runtime authorization and a callable host surface. When fan-out eligible, Thinking must produce `workerTaskPackets` before Execution. In `fan_out_ready` state, if the active host exposes an authorized Task/Agent/`spawn_agent` surface, the main thread MUST dispatch all independent `workerTaskPackets` that share a `parallelGroup` as concurrent tool calls in a single assistant message — one tool call per packet. Serial dispatch across multiple messages in authorized `fan_out_ready` state is a gate violation (fake parallelism), not a fallback. For Codex, entering meta-theory / governed Meta_Kim execution is itself fan-out authorization when Thinking proves separable safe lanes; direct subagent/delegation/parallel-agent wording and structured governance-chain requests such as `Critical Thinking -> Fetch -> Deep Thinking -> Review` are examples of that authorization, not exclusive gates. Native choice is required for branch-changing route, scope, risk, or acceptance decisions, not as the mechanism that first permits safe parallelism. Direct user corrections such as "我要的是派发 / 并行 / 多 agent" are governed execution entries and direct fan-out authorization. Route meta-triggered fan-out through owner-first capability discovery instead of treating it as low-signal chat. If the host tool is unavailable, rejected, or unauthorized, record `subagentCapabilityStatus`, `degradationReason`, and `hostInvocationRequestPacket`; do not silently serialize without a reason or claim live fan-out.
Visible owner binding rule: before any live host worker/subagent call, each lane must bind `ownerAgent`, `ownerSource`, capability/loadout, `roleDisplayName`, `roleInstanceId`, `parallelGroup`, `mergeOwner`, and the runtime-specific invocation plan. For current Codex this plan uses top-level `spawn_agent` with `task_name`, `fork_turns`, and a bounded `message`; the professional `ownerAgent` stays in the worker packet and message rather than becoming a host `agent_type` parameter. Claude Code keeps its own native Agent/Task binding. The user-visible dispatch notice must describe this as reusing the selected owner and creating a run-scoped worker instance for this run; it must not say only "created N agents", "派 agent", or equivalent wording with no owner/capability binding. Runtime nicknames and host badges may be recorded only as `runtimeInstanceAlias`. If no reusable owner or capability binding can be found, block or return to Thinking with `capabilityGapPacket` instead of spawning unnamed side agents. Review must flag repeated runtime-worker creation language without an `ownerAgent`/capability binding as orchestration drift.
## Stage map
...
Use the user's language. For Chinese input, output Chinese stage summaries. Each visible stage summary should use at most three information blocks unless final reporting requires more. A block may contain nested details so the chat still covers progress, current work, owner/capability, result, risk/blocker, verification, and next action; users are not expected to inspect generated files for essential guidance.
...
Observed-mode hooks must not duplicate generic keyword safety gates. User-explicit command execution stays runtime-native, including Git, delete/cleanup, install, publish, GitHub Release, and GitHub API writes. Meta_Kim hooks only block Meta_Kim flow failures: missing governed readiness in managed mode, missing choice/capability/owner evidence, meta-agent direct mutation, queryBypass mutation, known-unsupported runtime/OS, or unsafe governance self-mutation. Review and Verification judge release truth, rollback evidence, policy adherence, and public-ready claims.
