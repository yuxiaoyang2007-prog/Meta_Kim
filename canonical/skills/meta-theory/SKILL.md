...
Dispatch to `meta-conductor` for dynamic business-flow blueprint and parallel lane orchestration, then `meta-warden` for synthesis. Conductor must classify the user's natural-language intent, choose lanes by evidence and dependency signals, record omitted lanes with reasons, and only then fan out worker tasks. Thinking to Execution must select `agent-teams-playbook` as the fan-out orchestration adapter when there are 2+ executable independent parallel worker lanes; fewer than 2 executable lanes record `not_required`. The playbook is an adapter after `workerTaskPackets` exist, not a replacement for Critical, Fetch, Thinking, owner selection, or verification planning. Cap default parallel waves at 5 agents and avoid role inflation. Independent sub-tasks must be parallelized when safe; avoid fake parallelism.
