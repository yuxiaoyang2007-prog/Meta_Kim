...
// Hook reminders always land on stderr for the local operator. Keep these
// strings local to avoid making every runtime hook depend on a separate i18n
// module.
const SKIP_REASONS = {
  env_var: (detail) => `Skipped via env var${detail ? `: ${detail}` : ""}`,
  keyword: () => "Skipped: simple-keyword detected in prompt",
  governance_flow: (detail) =>
    `Skipped by governance flow${detail ? `: ${detail}` : ""}`,
};
const SKIP_REASON_UNKNOWN = (source) =>
  `Skipped: unknown source (${source})`;
const HOOK_IMPACTS = {
  "enforce-agent-dispatch": "Potential loss of agent-dispatch governance",
  "post-format": "Potential code style issues",
  "post-typecheck": "Potential type errors",
  "stop-console-log-audit": "Leftover console.log may ship to production",
  "stop-save-progress": "Cross-session progress not persisted",
  "stop-memory-save": "Session observations not captured to MCP Memory",
};
...
export function formatSkipReason(source, detail = "") {
  const fn = SKIP_REASONS[source];
  if (fn) return fn(detail);
  return SKIP_REASON_UNKNOWN(source);
}
...
export function getHookImpact(hookName) {
  return HOOK_IMPACTS[hookName] || HOOK_IMPACT_GENERIC;
}
