...
默认直接回车的路径是：**全局通用能力 + 当前项目按目标平台选择的完整投影**。默认目标平台是 Claude Code + Codex，所以 setup 会为这两个工具安装可复用的全局 skill，并且只为这两个目标生成/更新项目级运行面：Claude Code 使用 `CLAUDE.md`、`.claude/` 和 `.mcp.json`；Codex 使用 `AGENTS.md`、`.codex/` 和 `.agents/skills/`。Cursor 和 OpenClaw 的项目文件只有在显式选择这些“正式投影兼容目标”时才会生成。依赖项目自己的安装目标留给上游项目说明；Meta_Kim 只在内部记录它们，防止被误升级成项目投影。任何 apply 过的项目 bootstrap 都会记录 `.meta-kim/` 状态与备份文件。

`global-only` 只是显式选择项。它只安装可复用 skill 和 meta-theory 入口，不会让所有项目自动进入 Meta_Kim 治理。在其他目录里，Meta_Kim 必须先展示项目 bootstrap dry-run 计划，并获得确认后才写入项目文件。已有用户配置会通过 managed block、add-only 写入、保护式 JSON merge、备份和 manifest 保留。
...
| 任意其他项目 + Claude Code | 全局 skill 可被发现；项目治理必须在本目录项目级投影确认/应用后才开始 | 直接说任务；`/meta-theory` 只是维护者快捷方式 |
| Codex | `AGENTS.md` 项目上下文 + 本地生成的 `.codex/agents/*.toml`、`.codex/hooks.json`、`.codex/commands/`、`.agents/skills/` | 直接说任务；Codex 会区分 durable 工作、主观模糊输入和纯查询 |
| OpenClaw | `openclaw/` workspace、共享 skill、template config 和 internal lifecycle hooks；`AGENTS.md` 只是团队/上下文材料，不是 OpenClaw 的统一入口 | 需要配置 OpenClaw config/auth；提交者必须先在 OpenClaw 自己完成严格自测并提供证据，证据通过审查后才能合并 |
| Cursor | `AGENTS.md` 可以贡献仓库上下文，但 Cursor 专属行为在 `.cursor/agents/`、`.cursor/rules/`、`.cursor/skills/`、hooks 和 MCP 镜像里 | 提交者必须先在 Cursor 自己完成严格自测并提供证据，证据通过审查后才能合并 |
...
| 显式正式兼容投影 | OpenClaw、Cursor | 只有显式选择目标时才生成项目文件；runtime 改造必须有对应工具端自测证据，才能视为完成。 |
...
Meta_Kim 把“全局可复用能力”和“按目录授权生效的治理”分开。全局安装（`node setup.mjs`）后，全局 skill 可以在任意项目被发现，但项目治理只在已经确认/应用项目级投影的目录中生效。下表区分可复用能力和必须回到 Meta_Kim 源仓库运行的校验：

| 执行层 | 全局安装后能用 | 需要 Meta_Kim 仓库 |
| --- | --- | --- |
| **Prompt 层**（agents + skills 中定义的门和协议规则） | 全局 skill 入口可复用；项目治理行为仍依赖本目录项目级投影 | — |
| **Hook 层**（会话结束时的门检查、记忆保存至 MCP Memory Service、危险命令拦截） | 只在 hooks/settings 被明确配置的目录或全局高级开关中生效；高级全局控制默认关闭 | — |
| **配置层**（workflow-contract.json 中的协议字段定义） | AI 可以读取已安装规则；项目文件必须在 bootstrap 确认后才写入 | — |
