...
Meta_Kim 就是干这个的。它不是另一个模型，而是编码工具之上的治理层：用可运行的 agents、skills、contracts、hooks、scripts 和证据门，把复杂任务从“一锅乱炖”变成可审查、可验证、可沉淀的执行。
...
Claude/Codex/Cursor/OpenClaw 四个工具端都是 Meta_Kim 的正式投影，但原生能力表面和证据等级不同。Claude Code 和 Codex 是默认选择的主链路；OpenClaw 与 Cursor 也是正式投影，只是非默认选择。增强 OpenClaw/Cursor 原生能力的 PR 需要提交者先在对应工具端自己完成严格自测并提供证据，证据通过审查后才能合并。投影 smoke、fixture 校验和生成报告都是有用证据，但不能冒充 native-live runtime proof。
...
| **治理闭环支持** | 通过 Claude-native 表面默认正式投影 | 通过 Codex-native 表面默认正式投影 | 通过 OpenClaw-native 表面非默认正式投影；typed plugin 工具阻断改造需提交者严格自测并提供证据，证据通过审查后才能合并 | 通过 Cursor-native 表面非默认正式投影；项目决策卡与官方 hook gate 保持原生语义 |
...
- **核心价值**：
  ...
  - **大幅降低幻觉**——agent 先从图谱候选锚点定位，再用真实源文件确认，而不是凭记忆瞎编
...
Claude Code 和 Codex 是默认正式工具端投影；OpenClaw 和 Cursor 是非默认正式投影。Qoder CLI、Trae、Kiro、Windsurf / Devin Desktop Cascade、Cline、Roo Code、Continue 目前是 candidate probe：官方文档确认它们有可映射的兼容原始能力，但 Meta_Kim 还没有把它们升级成正式工具端投影。依赖项目自己的安装目标由上游项目维护，不重复写成 Meta_Kim 的支持承诺。精确边界见 `config/runtime-compatibility-catalog.json`。
