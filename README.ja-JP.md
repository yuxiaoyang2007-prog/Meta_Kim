...
| explicit formal compatibility projection | OpenClaw、Cursor | 明示的に選択した場合だけ project projection を生成します。runtime 変更は、その tool 側の self-test evidence が必要です。 |
...
Meta_Kim は現在 4 つの正式 projection target を持ちます。
...
4 つの正式 target は同じ canonical source から生成されますが、native surface は異なります。Claude Code と Codex は default formal projection、OpenClaw と Cursor は explicit formal compatibility projection です。
